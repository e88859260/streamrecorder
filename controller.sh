...
STREAMLINK=$(grep "^Streamlink" ./config/"$1".config|awk -F = '{print $2}')
MEGAPAN=$(grep "^Megapan" ./config/"$1".config|awk -F = '{print $2}')
...
[[ ! -d "${SAVEFOLDER}streamlink" ]]&&mkdir ${SAVEFOLDER}streamlink
[[ ! -d "${SAVEFOLDER}streamlink/metadata" ]]&&mkdir ${SAVEFOLDER}streamlink/metadata
[[ ! -d "${LOGFOLDER}streamlink" ]]&&mkdir ${LOGFOLDER}streamlink
sleep 5
./recorder.sh streamlink $STREAMLINK $1 ${SAVEFOLDER}streamlink/ ${LOGFOLDER}streamlink/ $FORMAT $LOOP $INTERVAL $STREAMORRECORD $RTMPURL &
sleep 10
fi
...
if [[ -n "$MEGAPAN" ]]; then 
sleep 5
./megadlpush.sh megapan $MEGAPAN $1 ${SAVEFOLDERGLOBAL}/ ${LOGFOLDER} $STREAMORRECORD $RTMPURL &
sleep 10
fi
